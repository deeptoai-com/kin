...
4. **安全护栏（见 §8）**：现状缺关键 env 保护（MCP 可覆盖 `ANTHROPIC_*`/`PATH`）、凭证明文、URL 无 scheme 校验。注：威胁模型 = 私有部署 / 半可信团队（§11#1），stdio 与连内网是**合法用途、不封**；护栏目标为「防误操作 + 共享宿主密钥卫生」。
...
- G3 **加合理护栏**（私有部署基线）：保护关键 env、凭证加密、stdio「服务器执行」警示——**而非**锁死 stdio / 连内网（那是合法核心用途）。
...
| D4 | **租户模型 = 私有化部署（中小团队 / 公司内部，半可信用户）**。stdio 视为**合法能力，普通用户可用**；护栏 = 在沙盒内执行 + 保护关键 env + UI 警示，**而非** admin 锁死 | §8；§11#1 已定 |
| D5 | 凭证**加密存储**（不再明文 JSON），UI 按 `sensitive` 脱敏 | §8（共享宿主 / 备份卫生） |
...
- 添加自定义：保留 手填 / URL / JSON-YAML / npm；**stdio 普通用户可用**（私有部署），但带「将在服务器执行外部命令、请确认来源可信」的醒目警示（见 D4/§8）。
...
## 8. 安全设计（威胁模型 = 私有部署 / 半可信团队）

**威胁模型（§11#1 已定）**：私有化部署、公司/团队内部、用户是**同事（半可信）**，**非公网匿名**。因此安全目标从「防匿名 RCE/SSRF」调整为：**① 防误操作伤共享宿主；② 共享宿主/备份的密钥卫生；③ 运行稳定（别让 MCP 配置搞坏会话）**。stdio、连内网都是**合法核心用途**，不封。

| 项 | 现状 | 处置（私有部署基线） |
|------|------|------|
| **stdio 命令** | 任意用户可加，无校验 | **保留为普通用户可用能力**（私有部署核心用途）。护栏：① 尽量在 **per-session 沙盒**内执行（复用现有 sandbox / bash-sandbox，防误操作/失控伤共享宿主，加固项见 P1/P2）；② UI 明确警示「该 MCP 会在**服务器**上执行外部命令，请确认来源可信」；③ 软提示（非 npx/uvx/python 等常见入口时给醒目提示）。**不做 admin 锁死、不做硬允许列表**——那会废掉主要价值。 |
| **http/sse URL** | 无校验 | 仅校验 **scheme（http/https）**。**不封私网/localhost**——私有部署常需连**内网/本地工具**，封禁=误伤合法用途。给谨慎团队一个**可选**的「出站域名/网段允许表」开关（默认关）。 |
| **凭证明文** | `credentials.json` 明文 | 加密存储（D5）；前端只见掩码；日志脱敏。动机是共享宿主/备份卫生，非防匿名。 |
| **env 变量名** | 可覆盖任意 env | **保护关键键**：禁止 MCP 的 `env` 覆盖 `ANTHROPIC_*`/`PATH`/`HOME`/`CLAUDE_*` 等（首要为**稳定性**——避免误改弄坏会话/泄漏宿主密钥）。其余键名放行。 |
| **工具门控** | allowedTools 传给 SDK，未验证强制 | 验收：确认 SDK 按 allowedTools 拒未授权工具；UI 标注被过滤项。 |
| **审计** | 无 | 最小审计：记录 enable/disable / 凭证变更 / 自定义 MCP 注册（团队内可追溯即可，完整审计后置）。 |

## 9. 分期

- **P0（收口 + 轻护栏）**：①MCP 收口进能力中心 MCP tab；②关键 env 保护（禁覆盖 `ANTHROPIC_*`/`PATH`/`HOME`/`CLAUDE_*`）；③stdio UI 警示 + 软提示；④URL scheme 校验。**纯增量、低风险、不依赖上游。**
- **P1（SDK 运行时状态）**：接 `mcpServerStatus`/`toggle`/`reconnect`，状态徽章 + 热启停 + 重连 + 错误展示。
- **P2（DB 编目 + 凭证加密）**：catalog/enablement/cred(密文)/override 迁入 DB + seeder + 迁移脚本；`resolveMcpServerConfigs` 改读 DB。
- **P3（加固/可选）**：stdio 走沙盒执行、可选出站域名允许表、最小审计、市场/评分/审批工作流。

> 备注：威胁模型已定为**私有部署/半可信团队**（§11#1），原「P0 上线阻塞 RCE」框架**已撤销**——stdio/连内网是合法用途。护栏目标转为「防误操作 + 密钥卫生」，不阻塞上线。
...
1. ~~租户模型~~ **【已定 2026-06】私有化部署（中小团队 / 公司内部，半可信用户），鼓励"自己小圈子用"，非公网大众。** → stdio 与连内网作为**合法核心用途**对普通用户开放；安全基线从"防匿名 RCE/SSRF"调整为"防误操作 + 共享宿主密钥卫生"（见 §8、§9）。
2. **凭证加密载体**：先做「加密文件」还是直接「DB 密文」？（影响 P2；倾向随 DB 编目一起做密文）
3. **stdio 沙盒**：是否在 P1/P2 把 stdio MCP 纳入现有 per-session sandbox 执行（加固项，非阻塞）。
...
- **改**：
  - `mcp.server.ts`：加关键 env 保护（禁覆盖 `ANTHROPIC_*`/`PATH`/`HOME`/`CLAUDE_*`）、URL scheme 校验、stdio「服务器执行」警示标记；凭证读写改加密；（P2）改读 DB。
  ...
  - `add-custom-mcp-dialog.tsx`：stdio 保留可用 + 加「服务器执行」警示（P0）。
