...
- **引擎 = 可插拔 VLM**（不钉死单一）：`OCR_PROVIDER` 抽象，与 `EMBED_PROVIDER` 同构。
  ...
  - `gemma`（OpenRouter，**已验证可用 2026-06-13**）：付费档 `google/gemma-4-31b-it`
    （`:free` 实测持续 429 不可用），`OPENROUTER_API_KEY` Bearer，`/api/v1/chat/completions`。
    实测 OCR 财报表格→markdown 成功，**~$0.00035/页**（几分钱级）。配置在仓外 secrets.env。
  - 两者都是 VLM，输入页图 → 输出 markdown，接口统一。
  - **实测重要发现（2026-06-13）**：VLM 在难认单元格会**编字**（gemma 把公司名识别成"华creases
    证券"、蹦出缅甸文）——证实 §6.1「质量徽章 + 左右对照」是刚需护栏（呼应 D5），非装饰。
...
| 阶段 | 范围 | 消费者 | 优先级 |
|---|---|---|---|
| **O1 引擎 + doubao + 双消费者接入** | `OCR_PROVIDER` 抽象 + doubao VLM（页图→markdown）；parser sidecar 加 `/render`（PDF→PNG）；**① `ocr` MCP 工具注册进 worker** + **② ingest 扫描件分支走 OCR → chunk/embed** | ①② | 高（堵 RAG 洞 + Agent 能读扫描件） |
| **O2 独立转换器（极简产品）** | 主栏第 4 模块 + `/agents/ocr` + 拖拽空状态 + 逐页进度 + 左右对照结果页 + **格式切换（MD/文本/表格/HTML，母格式本地派生）** + 复制/导出/加入知识库 | ③ | 高（独立产品入口） |
